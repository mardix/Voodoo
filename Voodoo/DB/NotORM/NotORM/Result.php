...
class NotORM_Result extends NotORM_Abstract implements Iterator, ArrayAccess, Countable {
	...
	*/
	function __construct($table, \Voodoo\DB\NotORM\NotORM $notORM, $single = false) {
		$this->table = $table;
		$this->notORM = $notORM;
		$this->single = $single;
	...
	*/
	...
	protected function createJoins($val) {
        ...
        /**
         * To execute query and return the object
         * @param   string $query - the query string to execute
         * @paran   mixed $parameters - parameters to pass to query
	...
	*/
	function offsetUnset($key) {
        ...
        /**
         * Reset everthing usually before a new query
         * @return \NotORM_Result 
         */
        public function reset(){
            foreach(array("select","conditions","where","parameters","order","union","unionOrder","data","referencing","aggregation","accessed","access","keys") as $k)
                  $this->$k = array();
            foreach(array("limit","offset","lock","unionLimit","unionOffset") as $k)
                   $this->$k = null;
            foreach(array("group","having") as $k)
                    $this->$k = "";
